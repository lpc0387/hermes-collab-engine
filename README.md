...
| **Skill 分发** | **v4.5** Leader 根据节点能力自动选择技能注入 Worker prompt，支持按需分发 |
| **MCP 工具管理** | **v4.5** Leader 根据节点类型和 Agent 能力分配工具白名单，含 MCP 只读工具 |
...
| **可视化面板** | **v4.5** 协同流水线视图：节点按状态分列，Worker 池卡片，Skill/Tool badge 实时展示 |
...
### 查看 Skill 和 Tool（v4.5）

```bash
hermes-collab skills                               # 列出所有注册技能
hermes-collab skills --node-type implementation     # 预览该节点能力下选中的技能
hermes-collab skills --node-type research --task "search and verify" --json

hermes-collab tools                                # 列出所有工具配置
hermes-collab tools --node-type implementation      # 预览该节点能力下选中的工具
hermes-collab tools --node-type verification --task "mcp read-only tests"
```
...
| GET | `/api/skills?node_type=&task=` | Skill 注册表（可选预览选择） |
| GET | `/api/tools?node_type=&task=` | Tool 配置（可选预览选择） |
