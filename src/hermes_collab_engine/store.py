...
class CollabStore:
    ...
    def insert_wbs_node(self, run_id: str, node: dict[str, Any]) -> None:
        self._execute(
            ...
            (node["id"], run_id, node.get("parent_id"), node["title"], node["description"], node["capability"], node["complexity"], json.dumps(node.get("dependencies", []), ensure_ascii=False), 1 if node.get("parallelizable", True) else 0, node["deliverable"], node.get("status", "pending"), node.get("attempt", 1)),
        )
    ...
    def run_detail(self, run_id: str) -> dict[str, Any]:
        ...
        return {"run": dict(run) if run else None, "nodes": [dict(r) for r in self._query("SELECT * FROM wbs_nodes WHERE run_id=? ORDER BY id", (run_id,))], "workers": [dict(r) for r in self._query("SELECT * FROM workers WHERE run_id=? ORDER BY started_at DESC", (run_id,))], "logs": [dict(r) for r in self._query("SELECT * FROM logs WHERE run_id=? ORDER BY id DESC LIMIT 200", (run_id,))]}
