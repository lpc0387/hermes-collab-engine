# Agent Communication Protocol (ACP-Collab v0.3)
...
形态：`lessons` 表行；v0.2 起要求 lesson 带明确 scope，避免把局部经验误用为全局规则。
...
### 2.5 Checkpoint 通道（Engine Scheduler → SQLite → Worker / Parent）

v0.3 引入 checkpoint 系统，用于在高复杂度或高风险节点执行前暂停、记录决策，并在安全条件满足时自动恢复调度。

方向：Engine Scheduler 写入 checkpoint 状态；Parent/Operator 可通过 CLI/API 写入决策；Worker 只接收已决策后的节点 prompt，不直接写 checkpoint。

触发：
- 节点 complexity / risk 达到 checkpoint 阈值，且 `RiskPolicy` 要求先审查。
- lesson 指示同类节点曾因范围过宽、超时或冲突失败，需要先 checkpoint。
- Parent/Operator 手动要求 checkpoint 某 run/node。
- redo-node 生成的新节点继承高风险来源，需要重新确认执行边界。
...
```python
{
  "run_id": "<run_id>",
  "node_id": "<wbs_node_id>",
  "reason": "high-complexity" | "risk-policy" | "lesson" | "manual" | "redo-followup",
  "risk_policy": {
      "max_auto_resume_risk": "low" | "medium" | "high",
      "checkpoint_timeout_seconds": 300,
      "require_manual_for": ["destructive", "cross-file-conflict", ...]
  },
  "evidence": {
      "complexity_score": <number>,
      "risk": "low" | "medium" | "high",
      "lesson_ids": ["<optional>"],
      "upstream_node_ids": ["<optional>"]
  },
  "decision": "pending" | "auto_resume" | "resume" | "redo" | "skip" | "fail",
  "decision_reason": "<required once decision != pending>"
}
```

Auto-resume：
- 低风险 checkpoint 可在 `RiskPolicy.max_auto_resume_risk` 范围内自动恢复，避免把安全、窄化节点卡成人工等待。
- 自动恢复必须写 `logs`，并保留 `decision=auto_resume` 与原因；Parent 最终报告应披露发生过自动 checkpoint。
- 超过 checkpoint timeout 的节点不得无限等待；Engine 必须按 policy 选择 auto-resume、fail、或请求人工介入，并写明原因。

Decision：
- `resume` / `auto_resume`：解除该 node 的 checkpoint pause，节点重新进入 ready 队列。
- `redo`：转入 Redo 通道，原节点保留审计结果，新节点带 lineage 与 redo reason。
- `skip`：标记节点跳过；下游按上游缺失降级处理。
- `fail`：标记节点失败；下游不得把其 result 注入为可信上游。

规则：
- Checkpoint 是调度状态，不是 Worker deliverable；不得要求 Worker 自己决定是否暂停运行。
- `checkpoint_paused_nodes` 等新状态必须在 run 完成、失败、中断或 engine shutdown 的 except/finally 路径清理。
- checkpoint 决策必须可审计：写 `logs`，并能由 status/API 读到。
- 高复杂度节点不能绕过 checkpoint 直接派发，除非 policy 明确允许且记录原因。

### 2.6 Redo 通道（Parent/Operator / Engine → SQLite → Scheduler）

v0.3 引入 redo-node 语义，用于在 checkpoint、失败复盘或人工核验后，以可追溯方式重做某个节点，而不是覆盖原节点结果。

方向：Parent/Operator 通过 CLI/API 请求 redo；Engine 也可根据 checkpoint decision 或 lesson 触发 redo；Scheduler 只调度新生成的 redo node。

触发：
- checkpoint decision 为 `redo`。
- Worker 失败、超时或 deliverable 被 Parent 核验为不可信。
- 上游 context 变化导致已完成节点需要按新边界重做。
- lesson-driven checkpoint evolution 要求同类节点改用更窄 scope 重试。
...
```python
{
  ...
  "redo_node_id": "<new_node_id>",
  "reason": "checkpoint-redo" | "failed-verification" | "timeout" | "scope-change" | "manual",
  "description_delta": "<what changed from source node>",
  "lineage": {
      "parent_id": "<source parent or source node>",
      "grandparent_id": "<optional original ancestor>",
      "redo_of": "<source_node_id>"
  },
  "side_effects": {
      "source_status": "superseded" | "failed" | "kept-for-audit",
      "dependents_requeued": ["<node_id>", ...],
      "lessons_written": ["<lesson_id>", ...]
  }
}
```

Side effects：
- 原节点结果不得被覆盖；应保留为 audit trail，并通过 `redo_of` / lineage 关联到新节点。
- 依赖原节点且尚未完成的下游节点应重新计算 ready 状态；已完成下游是否 redo 由 Parent/Operator 或 policy 决定。
- redo node 必须继承 allowlist、risk policy、上游 context 规则，并明确 `description_delta`，禁止原样复制导致重复失败。
- 若 redo 来自失败核验，Engine 可写 lesson，但 scope 默认不超过 `node` 或 `wbs-family`。

规则：
- Redo 是新节点，不是 retry attempt；retry 保持同一 node description，redo 必须说明边界或证据变化。
- redo 的所有状态变更必须走 CLI/API/Engine 控制面，禁止直接改 SQLite。
- `paused_runs`、`checkpoint_paused_nodes` 与 redo 临时队列必须在异常退出和 engine shutdown 时清理，避免陈旧 pause 阻塞后续 run。

### 2.7 Dispatch-Control 通道（Engine Scheduler / Parent Intervention → SQLite）

v0.3 将调度控制提升为协议通道，覆盖 stream-scheduled dispatch、proactive split、pause/resume-run 与 intervention CLI。
...
| 干预 | 目标 | 语义 |
|---|---|---|
| pause/resume | run | 暂停或恢复新节点 dispatch；不杀已运行 worker，除非另有 cancel；恢复时必须清理 `paused_runs` |
| cancel/fail | run/node | 标记未完成工作为 failed，并写 reason |
| checkpoint/resume | node | 暂停或恢复 checkpointed node；低风险可按 policy auto-resume |
| retry | node | 以同一 node description 重新入队，attempt + 1 |
| redo | node | 生成新的 redo node，保留原节点审计结果并写 `redo_of` lineage |
...
### 2.8 Observability 通道（Engine → SQLite/Dashboard / Parent）
...
Parent (Hermes)
  ...
  │       │
  │       ├─ optional checkpoint(high-risk node)
  │       │       ├─ auto/manual resume → ready queue
  │       │       └─ redo decision → new redo node with redo_of lineage
  │       │
  ...
  │             ──► final report
  │
  ├── optional: hermes-collab intervene ...  (pause/resume-run/checkpoint/redo/retry/split/skip/cancel)
...
| Checkpoint 超时 | 按 RiskPolicy auto-resume / fail / 请求人工介入，并写 decision_reason | Engine / Parent |
| Redo 节点生成失败 | 保留 source node 原状态，记录错误；不得覆盖原 result | Engine / CLI |
| `lesson add --evidence-json` 非法 | 退出码 2 + 错误信息 | CLI |
| 干预 CLI 被拒绝或参数非法 | 不修改状态；返回非 0 并记录/显示原因 | CLI |

## 5. Schema 版本

当前协议版本：`ACP-Collab v0.3`。变更约定：
...
v0.3 相对 v0.2 的新增协议点：
- Checkpoint channel（高复杂度/高风险节点暂停、auto-resume、decision payload）。
- Redo channel（redo-node、`redo_of` lineage、保留原节点审计结果）。
- pause/resume-run 与 checkpoint/redo 干预语义。
- RiskPolicy 驱动的 checkpoint timeout 与自动恢复。
- lesson-driven checkpoint evolution（从 scoped lesson 调整 checkpoint/redo 策略）。
...
10. ❌ 高复杂度 / 高风险节点不设 checkpoint 直接派发——这会把本应审查的风险推给 Worker；必须按 RiskPolicy checkpoint、拆分或记录豁免原因。
